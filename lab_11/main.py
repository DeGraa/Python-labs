...
def get_data_part_of_name_and_lastname(partofname_pattern, lastname_pattern):
    ...
    try:
        ...
        cur.execute(
            "SELECT user_name, phone_number FROM Phonebook WHERE user_name LIKE %s OR user_name LIKE %s ORDER BY user_name",
            (f"%{partofname_pattern}%", f"% {lastname_pattern}"),
        )  # 1st seeks rows that have users  that contain part of name,2nd seeks for the rows that ends with some string
        rows = cur.fetchall()
...
def get_data_phonenum(phone_number_pattern):
    ...
    try:
        ...
        cur.execute(
            "SELECT user_name, phone_number FROM Phonebook WHERE phone_number LIKE %s ORDER BY user_name",
            (f"%{phone_number_pattern}%",),
        )  # contains some digits in phonenum
        rows = cur.fetchall()
...
def get_data_by_exact_phonenumber(phone_number):
    ...
    try:
        ...
        cur.execute(
            "SELECT user_name, phone_number FROM Phonebook WHERE phone_number = %s ORDER BY user_name",
            (phone_number,),
        )
...
def query_with_pagination(limit, offset):
    ...
    try:
        ...
        cur.execute(
            "SELECT user_name, phone_number FROM Phonebook ORDER BY user_name LIMIT %s OFFSET %s ",
            (limit, offset),
        )
        rows = cur.fetchall()
