...
def draw_menu(color):
    ...
    mid_right = (
        (top_right[0] + bottom_right[0]) // 2 - 4,
    ...
    eraser = pygame.image.load("lab_9/paint/img/eraser-square-svgrepo-com.svg")
...
def draw_painting(paints):
    for color, pos, figure in paints:
        if color == (255, 255, 255):  # If the color is white
            pygame.draw.rect(
                screen, color, [pos[0] - 15, pos[1] - 15, 37, 50]
            ...
            if figure == 0:  # If the figure is a circle
                ...
                pygame.draw.rect(
                    screen, color, [pos[0] - 15, pos[1] - 15, 37, 20]
                )  # Draw a rectangle
            elif figure == 2:  # If the figure is a square
                pygame.draw.rect(
                    screen, color, [pos[0] - 15, pos[1] - 15, 35, 35]
                )  # Draw a square
                ...
                pygame.draw.polygon(
                    screen, color, [vertex1, vertex2, vertex3]
...
while run:
    ...
    if mouse[1] > 85:
        ...
        if active_color == (255, 255, 255):
            ...
            pygame.draw.rect(
                screen, active_color, [mouse[0] - 15, mouse[1] - 15, 37, 50]
            )
        else:
            ...
            if active_figure == 0:  # Circle
                ...
                pygame.draw.rect(
                    screen, active_color, [mouse[0] - 15, mouse[1] - 15, 37, 20], 2
                )
            elif active_figure == 2:  # Square
                pygame.draw.rect(
                    screen, active_color, [mouse[0] - 15, mouse[1] - 15, 35, 35], 2
                )
                ...
                pygame.draw.polygon(
                    screen, active_color, [vertex1, vertex2, vertex3], 2
                )
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:  # If the user presses a mouse button
            for i in range(len(colors)):  # Iterate over the color rectangles
                if colors[i].collidepoint(
                    event.pos
                ):  # If the mouse click is within the color rectangle
                    active_color = rgbs[
                        i
                    ]  # Set the active color to the corresponding RGB value

            for i in brushes:  # Iterate over the brush icons
                if i[0].collidepoint(
                    event.pos
                ):  # If the mouse click is within the brush icon
                    active_figure = i[
                        1
                    ]  # Set the active figure to the corresponding brush type
